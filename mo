...
local function CreateGUI()
    ...
    local function createFeatureToggle(featureName, description, defaultState)
        ...
        ToggleButton.MouseButton1Click:Connect(function()
            ...
            elseif featureName == "ESPDistance" then
                if Features.ESP then
                    ClearESP()
                    CreateESP()
                end
            elseif featureName == "ESPGun" then
                if Features.ESP then
                    ClearESP()
                    CreateESP()
                end
...
function AddESP(character, player)
    ...
    if Features.ESPDistance then
        ...
        if head then
            ...
            billboard.Name = player.Name .. "_Distance"
            ...
            distanceConnection = RunService.Heartbeat:Connect(function()
                if character and character.Parent and head and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head") then
                    local playerHead = LocalPlayer.Character.Head
                    local distance = (head.Position - playerHead.Position).Magnitude
    ...
    if Features.ESPGun then
        local function checkForGuns()
            for _, tool in pairs(character:GetChildren()) do
                if tool:IsA("Tool") and not tool:FindFirstChild("GunESP") then
                    ...
                    gunHighlight.Name = player.Name .. "_Gun"
        ...
        checkForGuns()
        local gunConnection = character.ChildAdded:Connect(checkForGuns)
        table.insert(ESPConnections, gunConnection)
    end
end
